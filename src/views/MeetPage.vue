...
<script setup>
  ...
  const eventTime = computed(() => {
    ...
    // Format: YYYY, MM (0-indexed), DD, HH, mm
...
//   })
  ...
  // Performance: use computed properties with native arithmetic for synchronized and efficient updates
  const days = computed(() => Math.floor(Math.max(0, diff.value) / (1000 * 60 * 60 * 24)))
  const hours = computed(() => Math.floor((Math.max(0, diff.value) / (1000 * 60 * 60)) % 24))
  const minutes = computed(() => Math.floor((Math.max(0, diff.value) / (1000 * 60)) % 60))
  const seconds = computed(() => Math.floor((Math.max(0, diff.value) / 1000) % 60))
