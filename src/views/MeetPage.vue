...
<script setup>
  ...
  // Vite's tree-shaking will remove date-fns from the production bundle as it's no longer imported.
  ...
  const diffSeconds = computed(() => {
    const diffMs = eventTime.value - now.value
  ...
  const minutes = computed(() => Math.floor((diffSeconds.value % 3600) / 60))
  const seconds = computed(() => Math.floor(diffSeconds.value % 60))
