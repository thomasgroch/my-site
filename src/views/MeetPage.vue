...
<script setup>
  ...
  const eventTime = computed(() => {
    ...
    const p = props.date.split('-').map(p => parseInt(p, 10))
  ...
  const itIsTime = computed(() => eventTime.value <= now.value)

  const timeDiff = computed(() => {
    return Math.max(0, Math.floor((eventTime.value - now.value) / 1000))
  })

  const days = computed(() => Math.floor(timeDiff.value / (24 * 3600)))
  const hours = computed(() => Math.floor((timeDiff.value % (24 * 3600)) / 3600))
  const minutes = computed(() => Math.floor((timeDiff.value % 3600) / 60))
  const seconds = computed(() => timeDiff.value % 60)
